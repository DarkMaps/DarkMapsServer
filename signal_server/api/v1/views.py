...
class MessageList(APIView):
    ...
    def get(self, request, **kwargs):
        ...
        logger.info(f"Get messages: Complete in {toc - tic:0.4f}s")
    ...
    def post(self, request, **kwargs):
        ...
        serializer.save()
        toc = time.perf_counter()
        logger.info(f"Post Messages: Complete in {toc - tic:0.4f}s")
    ...
    def delete(self, request, **kwargs):
        ...
        toc = time.perf_counter()
        logger.info(f"Delete Message: Complete in {toc - tic:0.4f}s")
...
class DeviceView(APIView):
    ...
    def post(self, request, **kwargs):
        ...
        serializer.save()
        toc = time.perf_counter()
        logger.info(f"Post Device: Complete in {toc - tic:0.4f}s")
    ...
    def delete(self, requested, **kwargs):
        ...
        device.delete()
        toc = time.perf_counter()
        logger.info(f"Delete Device: Complete in {toc - tic:0.4f}s")
...
class PreKeyBundleView(APIView):
    ...
    def get(self, request, **kwargs):
        ...
        toc = time.perf_counter()
        logger.info(f"Get Prekey Bundle: Complete in {toc - tic:0.4f}s")
        # Return bundle
        return Response(serializer.data, status=status.HTTP_200_OK)
...
class UserPreKeys(APIView):
    ...
    def post(self, request, **kwargs):
        ...
        try:
            ...
            toc = time.perf_counter()
            logger.info(f"Post Pre-Keys: Complete in {toc - tic:0.4f}s")
...
class UserSignedPreKeys(APIView):
    ...
    def post(self, request, **kwargs):
        ...
        serializer.save()
        toc = time.perf_counter()
        logger.info(f"Post Signed Pre-Key: Complete in {toc - tic:0.4f}s")
...
def user_delete_callback(sender, **kwargs):
    ...
    logger = logging.getLogger("watchtower")
    logger.info(f"User Deleted - Total number of users: {userCount}")
...
def user_registered_callback(sender, **kwargs):
    ...
    userCount = userModel.objects.count()
    logger = logging.getLogger("watchtower")
    logger.info(f"User Registered - Total number of users: {userCount}")
